...
(* ---- MANUAL STUBS (issue #58): std types the Coq backend omits but the
   generated ESS code references. Opaque — only their existence is needed. ---- *)
Definition core_convert_From (Self Source : Type) : Type := unit.
Definition core_convert_Infallible_t : Type := Empty_set.
Definition core_slice_iter_Iter_t (T : Type) : Type := list T.
Definition core_slice_iter_IterMut_t (T : Type) : Type := list T.
Definition core_ops_range_Range_t (T : Type) : Type := (T * T)%type.
