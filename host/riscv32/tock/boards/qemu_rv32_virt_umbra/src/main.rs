...
// Embedded Umbra enclave: Tock drives the enclave API. `fibonacci` is the
// encrypted-in-place payload; `umbra_enclave` is the S-mode loader that `ecall`s
// the monitor to create/enter/read it.
mod fibonacci;
mod umbra_enclave;
...
pub unsafe fn main() {
    ...
    // Drive the embedded Umbra enclave to completion BEFORE Tock arms its
    // scheduler timer, so the enclave-preemption timer and Tock's vtimer never
    // contend for the real `mtimecmp`. Prints `[TOCK] enclave R0=0x72CA33A8` via
    // the monitor UART.
    umbra_enclave::drive_enclave();
