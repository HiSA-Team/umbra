...
The existing L552 vs N657 PAL crates (`src/hardware/platform/stm32l552/`
and `src/hardware/platform/stm32n657/`) are the two worked examples
that span the two ends of the question matrix above (Cortex-M33 with
internal flash + dual-bank vs Cortex-M55 FSBL on external XSPI).
...
src/hardware/platform/<mcu>/
  boot/                              # binary crate, the FSBL / Secure Boot
    ...
    src/platform_impl/               # PlatformBoot impl — decomposed by concern
      mod.rs                         # facade
      boot.rs                        # init sequence
      dma.rs                         # DMA wiring
      power.rs                       # PWR / sleep
      syscall_dispatch.rs            # SVC handler routing
    src/secure_kernel/               # ESS-residency lifecycle (L552 layout;
      mod.rs                         #   on N657 this can stay as a single
      init.rs                        #   secure_kernel.rs while it fits under
      enter.rs                       #   the 600-LOC hard-cap)
      exit.rs
      lifecycle.rs
    src/handlers.rs                  # exception handlers + raw_print on fault
    src/api_impl/                    # umbra_*_imp NSC API impls — one file per veneer
      mod.rs
      arg_validation.rs
      debug_print.rs
      enclave_create.rs
  ...
  drivers/
    ...
    src/hash/tests.rs                # split per hard-cap (mod tests;)
    src/dma.rs                       # request queue, controller
    src/dma/copier.rs                # umbra-hal::Dma trait adapter
    src/dma/tests.rs                 # split per hard-cap
    src/aes/                         # ecb / ctr / keyreg / gcm / hal_adapter
...
peripheral bases and register layouts. The submodule layout shown above
mirrors the live L552 structure; start with single-file `.rs` units if
your platform stays well under the 600-LOC hard-cap and decompose later
only when needed.
...
crate**, not in any rlib. Cargo silently ignores
`cargo:rustc-link-arg` emitted from a library crate's `build.rs`, so
the only way to forward `.s` files into the link is via the boot
