...
fn commit_all(store: &mut MockStore, anchor: &mut MockAnchor, seeds: &[u8; N]) {
    ...
    let dirty = 0xFFFFu16;
    checkpoint(store, anchor, dirty, &KEY, EID, mock_hmac).unwrap();
}
...
fn full_checkpoint_then_restore_resumes() {
    ...
    assert_eq!(restore(&s, &a, &KEY, EID, mock_hmac), RestoreDecision::Resume);
}
...
fn partial_checkpoint_preserves_nondirty_sectors() {
    ...
    checkpoint(&mut s, &mut a, 0b1, &KEY, EID, mock_hmac).unwrap();
...
fn replayed_old_coherent_checkpoint_is_rejected() {
    ...
    assert_eq!(restore(&s, &a, &KEY, EID, mock_hmac), RestoreDecision::Reject);
}
...
fn cold_anchor_is_genesis() {
    ...
    assert_eq!(restore(&s, &a, &KEY, EID, mock_hmac), RestoreDecision::ColdGenesis);
}
...
fn flash_write_failure_does_not_advance_anchor() {
    ...
    assert_eq!(checkpoint(&mut s, &mut a, 0b100, &KEY, EID, mock_hmac), Err(CheckpointError::FlashWrite(2)));
