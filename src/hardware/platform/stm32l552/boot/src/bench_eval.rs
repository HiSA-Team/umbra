...
#[cfg(all(feature = "bench-eval", not(feature = "i_acknowledge_benchmark_is_research_only")))]
compile_error!(
...
mod imp {
    ...
    /// No-op when bench-eval is disabled.
    #[inline(always)]
    pub fn record_boot_sec_cycles(_cycles_elapsed: u32) {}

    /// No-op when bench-eval is disabled.
    #[inline(always)]
    pub fn record_switch_cycles(_cycles_elapsed: u32) {}

    /// No-op when bench-eval is disabled — returns 0 so any bracket
    /// arithmetic computes a zero delta (the bracket itself is also
    /// cfg-gated in api_impl.rs, so this branch is unreachable in
    /// practice).
    #[inline(always)]
    pub fn read_cycles() -> u32 {
        0
    ...
    /// Empty zero-sized type so api_impl.rs can declare the guard
    /// under a cfg gate and have it compile unconditionally. The
    /// `Drop` impl is empty.
    pub struct SwitchGuard;

    impl SwitchGuard {
        #[inline(always)]
        pub fn start() -> Self { SwitchGuard }
    }
}

pub use imp::{init, record_boot_sec_cycles, record_switch_cycles, read_cycles, dump, SwitchGuard};
