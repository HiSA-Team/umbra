...
// Alias to the single-source-of-truth SLOT_SIZE in the kernel
// build-time knob via.cargo/config.toml [env]). We use the
// `_USIZE` variant generated by build.rs because the struct field
// `[u8; CODE_BLOCK_SIZE]` requires a `usize` constant.
pub use kernel::common::ess::SLOT_SIZE_USIZE as CODE_BLOCK_SIZE;
pub const BLOCK_META_SIZE: usize = 32;

pub struct ValidatedBlock {
    pub block_id: u32,
    #[allow(dead_code)]
    // Populated by validator; will be used when install path reads from ValidatedBlock directly
