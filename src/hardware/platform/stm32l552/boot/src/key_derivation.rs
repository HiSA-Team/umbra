...
//! Formal-model cross-ref: the L552 ProVerif model declares
...
/// `UmbraError::HashHardware` if the HASH engine wedges — callers must NOT
/// swallow it (a silent failure would leave `enc_key` all-zero).
...
/// Derive the chained-measurement HMAC subkey from `MASTER_KEY`. See
/// [`derive_enc_key`] for the error-propagation contract.
...
/// `[24..32]` discarded. Propagates `UmbraError::HashHardware` on HASH
/// timeout — a swallowed error here would derive the wrong OTFDEC key and
/// silently mis-decrypt every enclave fetch.
#[cfg(feature = "stm32l562")]
pub fn derive_otfdec_raw(crypto: &mut dyn CryptoEngine) -> UmbraResult<[u8; 32]> {
    let mut out = [0u8; 32];
    crypto
        .hmac(&MASTER_KEY, OTFDEC_KEY_LABEL, &mut out)
        .map_err(|_| UmbraError::KeyDerivation)?;
    Ok(out)
}
