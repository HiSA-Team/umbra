...
//! Software AES-128 with 4× 1 KB T-tables (the production L552 path).
//! Encrypt is the T-table fast path; decrypt stays byte-wise per the
//! parent module docs.
...
impl AesEmulated {
    ...
    fn key_expansion(&mut self) {
        ...
        while i < 4 {
            self.expanded_key[i] =
                u32::from_be_bytes(self.key[i * 4..(i + 1) * 4].try_into().unwrap());
            i += 1;
        }
    ...
    fn inv_mix_columns(state: &mut [u8; 16]) {
        for i in 0..4 {
            ...
            state[offset] =
                Self::gmul(c0, 14) ^ Self::gmul(c1, 11) ^ Self::gmul(c2, 13) ^ Self::gmul(c3, 9);
            state[offset + 1] =
...
mod tests {
    ...
    //! `AesEmulated` has no MMIO dependency — generates its own S-box +
    //! T-tables at runtime — so the same byte-for-byte path that runs on
    //! L552 also runs on the host. NIST FIPS-197 Appendix A vectors are
    ...
    /// Round-trip: encrypt(decrypt(x)) == x and decrypt(encrypt(x)) == x.
    /// Also independently exercises `decrypt_block` (byte-wise path) which
    /// the NIST encrypt-only KAT does not cover.
    #[test]
    fn aes128_ecb_round_trip() {
        ...
        assert_eq!(pt2, plaintext, "decrypt(encrypt(x)) != x");
    }

    /// Sanity: all-zero key + all-zero plaintext is a well-known fixed
    /// point of the AES-128 forward cipher. Expected ciphertext from
    /// a portable reference implementation:
    /// AES-128(K=0, P=0) = 66e94bd4ef8a2c3b884cfa59ca342b2e
    #[test]
    fn aes128_ecb_all_zeros() {
        let key = [0u8; 16];
        ...
        let mut out = [0u8; 16];
        aes.encrypt_block(&plaintext, &mut out);
        assert_eq!(out, expected, "AES-128(K=0, P=0) diverged from reference");

        // And the reverse direction must recover the all-zero plaintext.
        let mut back = [0u8; 16];
        aes.decrypt_block(&out, &mut back);
