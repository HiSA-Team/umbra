...
// module exports a no-op stub so the linker sees a defined symbol and
...
pub unsafe fn secure_boot() -> ! {
    ...
    if crate::boot_measurements::MODEL_BYTECODE_LEN != 0 {
        ...
        match kernel.measure_boot_blobs(&mut hash) {
            Ok(()) => {
                crate::raw_print::print_str("[UMBRASecureBoot] model+weights HMAC OK\r\n");
            }
            Err(_msg) => {
                crate::raw_print::print_str("[UMBRASecureBoot] model+weights HMAC FAIL — halt\r\n");
                loop {}
            }
        }
    }
...
#[no_mangle]
pub extern "C" fn DMA1_Channel2_Handler() {
    handle_dma_irq(1);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel3_Handler() {
    handle_dma_irq(2);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel4_Handler() {
    handle_dma_irq(3);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel5_Handler() {
    handle_dma_irq(4);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel6_Handler() {
    handle_dma_irq(5);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel7_Handler() {
    handle_dma_irq(6);
}
#[no_mangle]
pub extern "C" fn DMA1_Channel8_Handler() {
    handle_dma_irq(7);
}
