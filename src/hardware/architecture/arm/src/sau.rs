...
impl SauDriver {
    ...
    pub unsafe fn create_region(&mut self, config: &SauRegionConfig) {
        ...
        let limit_addr: u32 =
            (config.limit_addr & 0xffffffe0) | (config.nsc << 1) as u32 | (config.en) as u32;
...
impl MemorySecurityGuardTrait for SauDriver {
    ...
    fn memory_security_guard_create(&mut self, memory_block_list: &MemoryBlockList) {
        let region_base_address: u32 = MEMORY_BLOCK_SIZE
            * (memory_block_list
                .get_memory_block()
                .get_block_base_address());
        let region_limit_address: u32 = MEMORY_BLOCK_SIZE
            * (memory_block_list.get_memory_block_list_size())
            + region_base_address;
            ...
            .get_block_security_attribute()
        {
            MemoryBlockSecurityAttribute::Untrusted => {
                security_attribute = 0x0;
            }
            MemoryBlockSecurityAttribute::Trusted => {
                return;
            } // This is a placeholder, since trusted regions definition in ARM are undefined
            MemoryBlockSecurityAttribute::TrustedGateway => {
                security_attribute = 0x1;
            }
        }
